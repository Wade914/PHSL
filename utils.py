...
def hyperedge_deletion(incidence_matrix, drop_r):
    num_hyperedges = incidence_matrix.shape[1]
    num_to_drop = int(drop_r * num_hyperedges)
    if num_to_drop > num_hyperedges:
        raise ValueError("Drop rate too high, would result in deleting more edges than exist.")
    hyperedges_to_drop = np.random.choice(num_hyperedges, size=num_to_drop, replace=False)
    new_incidence_matrix = np.copy(incidence_matrix)
    for hyperedge_idx in hyperedges_to_drop:
        new_incidence_matrix[:, hyperedge_idx] = 0
...
def accuracy(preds, labels,epoch):
        preds = np.where(F.sigmoid(preds.cpu()) > 0.5, 1, 0)
        # print(preds)
        # preds = np.where(preds.cpu() > 0.5, 1, 0)
        # preds = preds.cpu().numpy()
        labels = labels.cpu().numpy()
        lbls_ = np.array(labels)
        # pred_class = torch.max(preds, 1)[1]
        preds_ = np.array(preds)
        TP = np.sum((lbls_ == 1) & (preds_ == 1))
        FP = np.sum((lbls_ == 0) & (preds_ == 1))
        FN = np.sum((lbls_ == 1) & (preds_ == 0))
        TN = np.sum((lbls_ == 0) & (preds_ == 0))
        running_corrects = (TP + TN) / (TP + FP + FN + TN)
        # if epoch==299:
