...
class HConstructor(nn.Module):
    ...
    def forward(self, inputs, args):
        ...
        for _ in range(self.iters):
            ...
            # Calculate softmax of nodes relative to edges
            q = self.to_q(edges)
            q = F.relu(q)
            ...
            # This determines which nodes to take edge features from
            ...
            # Softmax of edges relative to nodes (after updating edges)
            q = self.to_q(inputs)
            k = self.to_k(edges)
            k = F.relu(k)
            v = F.relu(v)
            ...
            # This determines how many edges a node belongs to
            attn_v = self.mask_attn(attn_v,args.k_e)
            H = attn_v
            
            # Calculate edge saturation level
            cc = H.ceil().abs()
            de = cc.sum(dim=0)
            empty = (de == 0).sum()
            s_level = 1 - empty/n_s
