...
def load_data2(args):
    ...
    path = osp.abspath(__file__)         # Current file absolute path
    d_path = osp.dirname(path)           # Current file directory
    # f_path = osp.dirname(d_path)         # Parent directory of current file directory
...
def load_ft(args, suffix=None):
    ...
    # Build file path based on suffix parameter
    if suffix is not None:
        suffix_str = str(suffix)
    else:
        suffix_str = "10"  # Default value, maintain backward compatibility
    
    # Build relative path, go up two levels to find exp directory
    base_path = osp.dirname(osp.dirname(osp.dirname(osp.abspath(__file__))))
    ...
    idx = np.array(idx)
    # edge_list = dhg.datapipe.load_from_pickle('data/data/H_no_FC.pkl')
    train_idx = np.where(idx == 1)[0]  # Training set indices
    test_idx = np.where(idx == 2)[0]
    val_idx = np.where(idx == 0)[0]
    fts = torch.Tensor(fts).to(device)
    
    # Keep labels as float type, because TDHNN project uses BCEWithLogitsLoss
    lbls = torch.Tensor(lbls).float().to(device)  # Change to float type
...
def load_data(args, suffix=None):
    if args.dataset in ['40','NTU']:
        return load_ft(args, suffix)
