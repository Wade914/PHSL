...
def arg_parse():
    ...
    parser.add_argument('--seed', type=int, default=int(random.randint(0, 100)))
...
def plot_embedding_2d(X, labels, fname, title=None):
    ...
    colors_space = np.linspace(0, 1, num_of_labels)           # Generate color space
    label_to_color = {}                                 # Map labels to colors
    ...
    cb=plt.colorbar(sc)
    cb.ax.tick_params(labelsize=32)  # Set colorbar scale font size
