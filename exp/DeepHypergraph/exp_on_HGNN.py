import os
# Set environment variable, must be before importing numpy and torch
...
if __name__ == "__main__":
    print("=" * 100)
    print("Starting multi-dataset, multi-hypergraph neural network model comparison experiment - each model runs 5 independent experiments on each dataset")
    print("=" * 100)
    
    # Define dataset suffixes to test
    dataset_suffixes = [30, 20, 10, 5, 0, -5, -10, -20, -30, -40, -50]
    
    # Define model list to test
    models_to_test = [
        # ("HGNN", lambda n_features, class_num: HGNN(n_features, 64, class_num)),
        ("HGNNP", lambda n_features, class_num: HGNNP(n_features, 64, class_num, use_bn=True)),
        ("HNHN", lambda n_features, class_num: HNHN(n_features, 64, class_num, use_bn=True)),
        ("HyperGCN", lambda n_features, class_num: HyperGCN(n_features, 64, class_num, use_bn=True))
    ]
    ...
    results_file = f"HGNN_experiment_results_{timestamp}.txt"
    
    with open(results_file, 'w', encoding='utf-8') as f:
        f.write("=" * 100 + "\n")
        f.write("Hypergraph Neural Network Model Multi-Dataset Comparison Experiment Results\n")
    ...
    for suffix in dataset_suffixes:
        ...
        for model_name, model_constructor in models_to_test:
            ...
            # *****************************************
                try:
                    X = pd.read_csv(f'data/X_{suffix}.csv', header=None, skiprows=1)
                    X = np.array(X)
                    
                    lbl = pd.read_csv(f'data/Y_{suffix}.csv', header=None, skiprows=1)
                    lbl = np.array(lbl)
                    class_num = int(lbl.shape[1])
                    
                    idx = pd.read_csv(f'data/idx_{suffix}.csv', header=None, skiprows=1)
                    idx = np.array(idx)
                    
                    edge_list = dhg.datapipe.load_from_pickle(f'data/H_{suffix}.pkl')
                ...
                for epoch in range(500):
                    ...
                    if epoch % 1 == 0:
                        with torch.no_grad():
                            ...
                            test_res = infer(net, X, HG, lbl, test_mask)
    ...
    print(f"{'Dataset':<15} {'Model':<12} {'Average Accuracy':<12} {'Std Dev':<8} {'Highest Accuracy':<12} {'Parameters':<12}")
    print("-" * 90)
    for result in summary_results:
        print(f"{result['dataset']:<15} {result['model']:<12} {result['mean_acc']:.4f}       {result['std_acc']:.4f}   {result['max_acc']:.4f}       {result['parameters']:,}")
    ...
    with open(results_file, 'a', encoding='utf-8') as f:
        ...
        f.write("#" * 100 + "\n")
        f.write(f"{'Dataset':<15} {'Model':<12} {'Average Accuracy':<12} {'Std Dev':<8} {'Highest Accuracy':<12} {'Parameters':<12}\n")
        f.write("-" * 90 + "\n")
        for result in summary_results:
            f.write(f"{result['dataset']:<15} {result['model']:<12} {result['mean_acc']:.4f}       {result['std_acc']:.4f}   {result['max_acc']:.4f}       {result['parameters']:,}\n")
