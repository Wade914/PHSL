...
def _generate_G_from_H(H, variable_weight=False):
    ...
    if not isinstance(H, torch.Tensor):
        H = torch.tensor(H, dtype=torch.float32).to('cpu')
        ...
        invDE = np.mat(np.diag(np.power(DE, -1))) .astype(np.float32)
        DV2 = np.mat(np.diag(np.power(DV, -0.5))) .astype(np.float32)
        W = np.mat(np.diag(W), dtype=np.float32)
        H = np.mat(H, dtype=np.float32)
        HT = H.T
        ...
        H = H.to(torch.float32)  # Convert to float32 data type
        ...
        # Create diagonal matrices for DE^-1 and DV^-1/2
        invDE = torch.diag_embed(DE.pow(-1))  # DE^-1
        DV2 = torch.diag_embed(DV.pow(-0.5))  # DV^-1/2
        ...
        if variable_weight:
            ...
            invDE_HT_DV2 = torch.matmul(invDE, torch.matmul(H.t(), DV2))
            return DV2_H, W, invDE_HT_DV2
        else:
            # Calculate G directly using PyTorch tensor operations
...
def construct_H_with_KNN_from_distance(dis_mat, k_neig, is_probH=True, m_prob=1):
    ...
    return H
def construct_H_with_KNN_from_distance_conv(dis_mat, k_neig, is_probH=True, m_prob=1):
    dis_mat = torch.tensor(dis_mat, dtype=torch.float32)
    n_obj = dis_mat.size(0)
    n_edge = n_obj
    H = torch.zeros((n_obj, n_edge), dtype=torch.float32, device=dis_mat.device)
    for center_idx in range(n_obj):
        dis_mat[center_idx, center_idx] = 0
        dis_vec = dis_mat[center_idx]
        _, nearest_idx = torch.sort(dis_vec)
        nearest_idx = nearest_idx[:k_neig].squeeze()
        avg_dis = torch.mean(dis_vec)
        if not torch.any(nearest_idx == center_idx):
...
def construct_H_with_KNN(X, K_neigs=[5], split_diff_scale=False, is_probH=True, m_prob=1):
    ...
    for k_neig in K_neigs:
        ...
        if not split_diff_scale:
            ...
            H.append(H_tmp)
    return H
def construct_H_with_KNN_conv(X, K_neigs=[5], split_diff_scale=False, is_probH=True, m_prob=1):


    if type(K_neigs) == int:
        K_neigs = [K_neigs]

    dis_mat = Eu_dis_conv(X)
    H = []
    for k_neig in K_neigs:
        H_tmp = construct_H_with_KNN_from_distance_conv(dis_mat, k_neig, is_probH, m_prob)
        if not split_diff_scale:
            H = hyperedge_concat(H, H_tmp)
        else:
            H.append(H_tmp)
